...
\`\`\`\n\n`;
    ...
    public async mostrarTareasProximas(diasProximos: number = 7): Promise<void> {
        try {
            ...
            contenido += this.generarBotonActualizacion("mostrarTareasProximas", diasProximos);
            contenido += `> [!info] Actualizado: ${hoy.toLocaleDateString()} ${new Date().toLocaleTimeString()}\n`;
    ...
    public async mostrarTareasStartProximas(diasProximos: number = 7): Promise<void> {
        try {
            ...
            // Utilizar el nuevo formato de agrupación por fechas
            contenido += this.renderizarTareasAgrupadas(tareas);
    ...
    public async mostrarTareasStartVencidas(): Promise<void> {
        try {
            ...
            if (tareasAgrupadas.criticas.length > 0) {
                ...
                // Usar fechaStart para la agrupación
                contenido += this.renderizarTareasAgrupadasPorInicio(tareasAgrupadas.criticas);
            }
    
            if (tareasAgrupadas.recientes.length > 0) {
                contenido += `## 🟠 Retrasadas ≤ 7 días (${tareasAgrupadas.recientes.length})\n\n`;
                // Usar fechaStart para la agrupación
                contenido += this.renderizarTareasAgrupadasPorInicio(tareasAgrupadas.recientes);
            }
    ...
    private agruparTareasPorFecha(tareas: Task[]): { [fecha: string]: Task[] } {
        ...
        try {
            ...
            tareas.forEach((tarea, index) => {
                ...
                console.log(`- Hora Inicio: ${tarea.horaInicio}`);
                
                // Para tareas con inicio vencido, usamos específicamente la fecha de inicio
                const fechaClave = tarea.fechaStart;
                console.log(`- Fecha clave para agrupación: ${fechaClave}`);
                
                if (!fechaClave) {
                    console.log("-> Ignorando tarea sin fecha de inicio");
                    return;
                }
    
                if (!grupos[fechaClave]) {
                    console.log(`-> Creando nuevo grupo para fecha ${fechaClave}`);
                    grupos[fechaClave] = [];
                }
                
                grupos[fechaClave].push(tarea);
                console.log(`-> Tarea agregada al grupo ${fechaClave}`);
                console.log(`-> Total tareas en este grupo: ${grupos[fechaClave].length}`);
            });
    
            console.log("\nGrupos creados:");
            Object.keys(grupos).forEach(fecha => {
                console.log(`- Grupo ${fecha}: ${grupos[fecha].length} tareas`);
            });
    
            // Ordenar tareas dentro de cada grupo
            Object.entries(grupos).forEach(([fecha, grupoTareas]) => {
                console.log(`\nOrdenando grupo ${fecha}`);
                console.log(`Tareas antes de ordenar: ${grupoTareas.length}`);
                TaskWeightCalculator.sortTasks(grupoTareas);
                console.log(`Tareas después de ordenar: ${grupoTareas.length}`);
            });
